...
import base64, os, uuid, json, datetime
...
def init_vault_table(eng):
    VaultFile.__table__.create(bind=eng, checkfirst=True)
...
def vault_preview(
    ...
    if ext == 'pdf':
        with open(f.filepath, 'rb') as fp:
            data = fp.read()
        return {"type": "pdf", "content": base64.b64encode(data).decode()}
...
def vault_combine(
    ...
    return {
        "message":           "Files combined using HE addition",
...
def vault_split(
    ...
    return {
        "message":  "Split back into original files",
        "restored": restored
    }